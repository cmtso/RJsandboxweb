...
scripts and notebooks (e.g.~R Markdown documents) with rich narratives
are deposited at a trusted data centre, (2) notebooks can be run
on-demand interactively as a web service, and (3) Shiny web apps provide
nice user interfaces to explore research outputs. However, notebooks
require users to go through the entire analysis, while Shiny apps do not
expose the underlying code and require extra work for UI design. We
propose using the `learnr' package to expose certain code chunks in R
Markdown so that users can readily experiment with them in guided,
editable, isolated, executable, and resettable code sandboxes. Our
...
In this paper, we describe a rapid approach to create and publish R
Markdown documents as ``interactive sandboxes'' using the
\CRANpkg{learnr} package, with the aim to bridge the gap between a
...
\CRANpkg{learnr} \citep{learnr} is an R package developed by RStudio to
...
is a key enabling technology for the \CRANpkg{learnr} package since it
...
session so that it can load quickly. To create a \CRANpkg{learnr}
tutorial in RStudio after \CRANpkg{learnr} is installed, the user
chooses a \CRANpkg{learnr} R Markdown template from a list after
...
the \CRANpkg{learnr} package are the ``exercise'' and ``quiz'' options.
...
skip sections. Like any Shiny apps, \CRANpkg{learnr} apps can be easily
embedded to other webpages, as seen in \citet{rmrwr}.

Although the \CRANpkg{learnr} package has existed for a few years now,
...
\CRANpkg{learnr} package to advance reproducible research, which we
...
\CRANpkg{learnr} allows users to create executable code boxes. Our
...
use the auto-graders and any quiz-like functionality of \CRANpkg{learnr}
...
Recognizing \CRANpkg{learnr} documents are themselves Shiny apps, our
...
\CRANpkg{learnr} document (Figure 1). Notice that it is very similar to
...
\item
  ...
  Markdown document
\item
  Open a new \CRANpkg{learnr} R Markdown template. Copy the content of
...
In general, \CRANpkg{learnr} tutorial apps can be published the same way
as Shiny web apps in Shiny servers, such as the ones provided by cloud
service providers or \url{https://shinyapps.io}. The \CRANpkg{learnr}
package vignettes
...
or data. We also note that future releases of \CRANpkg{learnr} will
...
benefit of not having to rely on \CRANpkg{RAppArmor}.
...
Since \CRANpkg{learnr} documents are inherently R Markdown documents
...
work. And since this is based on \CRANpkg{learnr} which is originally
...
\CRANpkg{learnr} package. It focuses on the R scripts and R Markdown
...
recommend the use of the \CRANpkg{shinymeta} \citep{shinymeta} package,
...
\CRANpkg{learnr} so we think it is more suitable for users that are
...
\CRANpkg{learnr} can run Python chunks (as well as other execution
